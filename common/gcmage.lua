...
-- Grips
local fire_grip = 'Fire Grip'
local earth_grip = 'Earth Grip'
local water_grip = 'Water Grip'
local wind_grip = 'Wind Grip'
local ice_grip = 'Ice Grip'
local thunder_grip = 'Thunder Grip'
local light_grip = 'Light Grip'
local dark_grip = 'Dark Grip'
...
local ElementalGripTable = {
    ['Fire'] = fire_grip,
    ['Earth'] = earth_grip,
    ['Water'] = water_grip,
    ['Wind'] = wind_grip,
    ['Ice'] = ice_grip,
    ['Thunder'] = thunder_grip,
    ['Light'] = light_grip,
    ['Dark'] = dark_grip   
}
...
function gcmage.EquipStaff()
    ...
    if (action.Skill ~= 'Enhancing Magic' and not ElementalDebuffs:contains(action.Name) and not string.match(action.Name, 'Utsusemi')) then
        ...
        if (DiabolosPoleSpells:contains(action.Name)) then
            if (environment.WeatherElement == 'Dark' and diabolos_pole) then gFunc.Equip('Main', 'Diabolos\'s Pole') 
            end
        end
...
function gcmage.EquipGrip()
    local action = gData.GetAction()
    local environment = gData.GetEnvironment()
    local player = gData.GetPlayer()

    if (action.Skill ~= 'Enhancing Magic' and not ElementalDebuffs:contains(action.Name) and not string.match(action.Name, 'Utsusemi')) then
        local grip = ElementalGripTable[action.Element]
        if grip ~= '' then
            gFunc.Equip('Sub', grip)
        end
    end
end
